...
class Node:
	...
	def process_leaf( self, all_words, my_words_idx, history ):
		# return self.minEntropy(all_words, my_words_idx)
		# return my_words_idx[0]
		return self.wordScore(all_words, my_words_idx)
	...
	def minEntropy( self, all_words, my_words_idx ):
		minEnt = 100000
		minWord = ""
		for word1 in my_words_idx:
			count = {}
			for word2 in my_words_idx:
				mask = self.reveal( all_words[word2], all_words[word1] )
				if mask not in count:
					count[ mask ] = []
				
				count[ mask ].append( all_words[word2] )
			currEnt = 0
			for x in count.values():
				p = len(x)/len(my_words_idx)
				currEnt += -1*(p)*math.log(p)
			if currEnt < minEnt:
				minEnt = currEnt
				minWord = word1
		return minWord

	def wordScore(self, all_words, my_words_idx):
		alphabet = "abcdefghijklmnopqrstuvwxyz"
		frequencies = {}
		for c in alphabet:
			freq = [0]*len(all_words[my_words_idx[0]])
			for idx in my_words_idx:
				for i in range(0,len(all_words[idx])):
					if all_words[idx] == c:
						freq[i] += 1
			frequencies[c] = freq
    
		max_freq = [0]*len(all_words[my_words_idx[0]])
		for c in frequencies:
			for i in range(0, len(all_words[my_words_idx[0]])):
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		if len( history ) == 0:
			...
			# query_idx = self.minEntropy(all_words, my_words_idx)
			# freq = self.letterFreq(all_words, my_words_idx)
			query_idx = self.wordScore(all_words, my_words_idx)
	...
	def fit( self, all_words, my_words_idx, min_leaf_size, max_depth, fmt_str = "    ", verbose = False ):
		...
		if len( my_words_idx ) <= min_leaf_size or self.depth >= max_depth:
			self.is_leaf = True
			self.query_idx = self.process_leaf( self.all_words, self.my_words_idx, self.history )
			if verbose:
				print( '█' )
		else:
			self.is_leaf = False
